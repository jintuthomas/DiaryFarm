 ...
 if(isset($b))
  {
    ...
    if($ch>0)
    {
        $sq = "UPDATE attent_report SET count=0, ch=0 WHERE staff_regid= '$b'";
    ...
    $sq2="select * from attent_report where name1='$f'";
...
if($rowcount==0)
{
    ...
    $d="INSERT INTO `attent_report`(`staff_regid`,`name1`, `name2`, `work`,`session`,`date`, `status`) 
    VALUES('$b','$f','$l','$w','$s','$da','Leave')";
...
else
{
    ...
    $sq = "UPDATE attent_report SET status='Leave',date='$da' WHERE staff_regid= '$b'";
    if(mysqli_query($db,$sq))
{
    echo '<script language="javascript">';
    echo 'alert("Attendance Marked successfully")';
